<script>
	...
	function AddSlide(){  // news_title  news_detail
		...
		var slide_title_th = $('#slide_title_th').val();
                var slide_detail = $('#slide_detail').val();
                var slide_detail_th = $('#slide_detail_th').val();
                var slide_desc = tinyMCE.editors[$('#slide_desc').attr('id')].getContent();
		$('#comment2').val(slide_desc);
                var slide_desc_th = tinyMCE.editors[$('#slide_desc_th').attr('id')].getContent();
		$('#comment3').val(slide_desc_th);
		var ImagesFiles = $('#ImagesFiles').val();
		var ImagesFiles1 = $('#ImagesFiles1').val();
		var img_old = $('#img_old').val();
		var Backgroundimg_old = $('#Backgroundimg_old').val();
		...
		}else if((ImagesFiles1=='')&&(Backgroundimg_old=='')){
				swal({
					title: 'Warning !',
					text: "Please enter Picture Background",
					type: 'warning',
					confirmButtonClass: 'btn btn-confirm mt-2'
			}) 
		}else{
	...
	//--------------------------- 
	function  loadbackImages(ProID){
		$.post('<?php echo base_url('control/loadSlidebackImages')?>' , { ProID : ProID }, function(data){
