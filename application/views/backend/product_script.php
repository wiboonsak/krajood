<script>
	...
	function Addproduct(){
		...
		var product_name_th = $('#product_name_th').val();
                var product_desc = tinyMCE.editors[$('#product_desc').attr('id')].getContent();
		$('#comment').val(product_desc);
                var product_desc_th = tinyMCE.editors[$('#product_desc_th').attr('id')].getContent();
		$('#comment_th').val(product_desc_th);
                var comment = $('#comment').val();
                var comment_th = $('#comment_th').val();
		var product_price = $('#product_price').val();                
		var product_category = $('#product_category option:selected').val();
                if(product_name == ''){
                                swal(
					{
						title: 'Please Enter Name English. !',
						confirmButtonClass: 'btn btn-confirm mt-2',
						type: 'warning'
					}
				)
